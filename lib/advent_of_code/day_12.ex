defmodule AdventOfCode.Day12 do
  ...
  def solve(input, part: part) do
    #   [??? ## ???? ##] 1, 3, 1, 2
    # - decrease by 1, skip one since reached 0
    #   [  ? ## ???? ##"] 3, 1, 2
    #   [  ?]
    ...
    |> Enum.map(fn line ->
      ...
      {sequence, counts} =
        case part do
          1 -> {sequence, counts}
          2 -> {sequence |> List.duplicate(5) |> List.flatten(), counts |> List.duplicate(5) |> List.flatten()}
        end
    ...
    if length(sequence) < length(counts) do
      ...
      cond do
        ...
        String.contains?(sequence_head, "#") ->
          cond do
            ...
            count_head > String.length(sequence_head) ->
              if length(sequence_tail) > 0 do
                acc *
                  total_arrangements(sequence_tail, [count_head - String.length(sequence_head) | count_tail], acc, true)
              else
                0
              end
          end
        ...
        sequence_head == "?" ->
          ...
          acc * (count_if_skipped + count_if_filled)
      end
    end
  end
